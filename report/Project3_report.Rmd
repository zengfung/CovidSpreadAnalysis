...
abc <- abc %>%  mutate(rate=New_deaths/Cumulative_cases, month=lubridate::month(Date_reported)) %>% select(-Country_code, WHO_region) # %>% dplyr::filter(!is.na(rate)) 
...
P_barc <- Poland %>% plot_ly(x=~Date_reported,y=~New_cases,color = ~stay_home_requirements) %>%
  ...
  layout(xaxis = list(title = "Date reported",
         ...
         yaxis = list(title = "Poland",
                      zeroline = FALSE)) 
subplot( F_barc, UK_barc, G_barc, I_barc, P_barc, nrows = 5,shareX = T, shareY = F, titleX = T,titleY=T) %>% layout(title = "Figure 3: Daily new cases of each country",showlegend=FALSE,showlegend2=FALSE)
